...
function buildRoleOptions(t: TFunction): { value: EvaluationRole | 'custom'; label: string }[] {
  return [
    ...(Object.keys(EVALUATION_ROLE_LABELS) as EvaluationRole[]).map((r) => ({ value: r, label: getRoleLabel(r, t) })),
    { value: 'custom', label: t('common.evaluation.templateBuilder.customRoleOption', 'Custom (no specific role)') },
  ];
}
...
export default function EvaluationTemplateBuilder({ onClose }: Props) {
  ...
  function startFromSample(sampleRole: EvaluationRole) {
    setEditing(null);
    setName(`${getRoleLabel(sampleRole, t)} Evaluation`);
    setRole(sampleRole);
  ...
  async function handleDelete(id: string) {
    if (!window.confirm(t('common.evaluation.templateBuilder.confirmDelete', 'Delete this evaluation template?'))) return;
  ...
  async function handleSave() {
    setError(null);
    if (!name.trim()) { setError(t('common.evaluation.templateBuilder.errors.nameRequired', 'Template name is required.')); return; }
    const cleanCriteria = criteria
      .map((c) => ({ ...c, label: c.label.trim(), description: c.description.trim() }))
      .filter((c) => c.label);
    if (cleanCriteria.length === 0) { setError(t('common.evaluation.templateBuilder.errors.criterionRequired', 'Add at least one criterion.')); return; }
    setSaving(true);
    try {
      ...
      setError(err instanceof Error ? err.message : t('common.evaluation.templateBuilder.errors.saveFailed', 'Failed to save template.'));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-950/50 p-4 overflow-y-auto">
      <div className="my-4 w-full max-w-2xl rounded-xl bg-white shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-bold text-slate-900">{t('common.evaluation.templateBuilder.title', 'Custom Evaluation Form Builder')}</h2>
            <p className="text-sm text-slate-500">{t('common.evaluation.templateBuilder.subtitle', 'Create custom scoring criteria for roles or ad-hoc evaluations.')}</p>
          </div>
        ...
        {!showEditor ? (
          <div className="p-5 space-y-4">
            ...
            >
              <Plus className="h-4 w-4" /> {t('common.evaluation.templateBuilder.newTemplate', 'New Custom Template')}
            </button>

            {/* Built-in role-based sample templates — one per role, never
                tied to a person. "Customize" copies it into the editor. */}
            <div>
              <p className="mb-2 text-sm font-semibold text-gray-700">{t('common.evaluation.templateBuilder.sampleTemplatesTitle', 'Role-Based Sample Templates')}</p>
                ...
                {(Object.keys(EVALUATION_ROLE_LABELS) as EvaluationRole[]).map((r) => (
                  <div key={r} className="flex items-center justify-between gap-3 rounded-lg border border-dashed border-gray-200 bg-gray-50 px-3 py-2.5">
                    <div className="min-w-0">
                      <p className="font-medium text-gray-900 truncate">{t('common.evaluation.templateBuilder.sampleTemplateName', '{{role}} Evaluation (Sample)', { role: getRoleLabel(r, t) })}</p>
                      <p className="text-xs text-gray-500">{t('common.evaluation.templateBuilder.sampleTemplateMeta', '{{role}} · {{count}} criteria · built-in', { role: getRoleLabel(r, t), count: ROLE_CRITERIA[r].length })}</p>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      {t('common.evaluation.templateBuilder.customize', 'Customize')}
                    </button>
                  </div>
                ))}
              </div>
            ...
            <p className="text-sm font-semibold text-gray-700">{t('common.evaluation.templateBuilder.customTemplatesTitle', 'Custom Templates')}</p>
            {loading ? (
              ...
              <div className="flex flex-col items-center py-10 text-center text-gray-400">
                ...
                <p className="text-sm">{t('common.evaluation.templateBuilder.noCustomTemplates', 'No custom templates yet.')}</p>
              </div>
            ) : (
              <div className="space-y-2">
                {templates.map((tpl) => (
                  <div key={tpl.id} className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 px-3 py-2.5">
                    <div className="min-w-0">
                      <p className="font-medium text-gray-900 truncate">{tpl.name}</p>
                      <p className="text-xs text-gray-500">
                        {t('common.evaluation.templateBuilder.templateMeta', '{{role}} · {{count}} criteria', {
                          role: tpl.role === 'custom'
                            ? t('common.evaluation.templateBuilder.customRoleFallback', 'Custom')
                            : (getRoleLabel(tpl.role, t) ?? tpl.role),
                          count: (tpl.criteria ?? []).length,
                        })}
                      </p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <button type="button" onClick={() => startEdit(tpl)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-blue-600">
                        <Pencil className="w-4 h-4" />
                      </button>
                      {canDelete && (
                        <button type="button" onClick={() => void handleDelete(tpl.id)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-red-600">
          ...
          <div className="p-5 space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('common.evaluation.templateBuilder.templateNameLabel', 'Template Name *')}</label>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder={t('common.evaluation.templateBuilder.templateNamePlaceholder', 'e.g. QC Inspector Evaluation')}
                  className="w-full min-h-11 rounded-lg border border-gray-200 px-3 text-sm focus:border-blue-400 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('common.evaluation.templateBuilder.appliesToRoleLabel', 'Applies to Role')}</label>
                <select
                  value={role}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-gray-700">{t('common.evaluation.templateBuilder.criteriaLabel', 'Criteria')}</label>
                <span className={`text-xs font-medium ${totalWeight === 100 ? 'text-emerald-600' : 'text-amber-600'}`}>
                  {t('common.evaluation.templateBuilder.totalWeight', 'Total weight: {{weight}}%', { weight: totalWeight })} {totalWeight !== 100 && t('common.evaluation.templateBuilder.totalWeightHint', ' (should sum to 100%)')}
                </span>
              </div>
                  ...
                  <div key={c.id} className="rounded-lg border border-gray-100 p-3 space-y-2">
                    <div className="flex items-start gap-2">
                      <input
                        ...
                        placeholder={t('common.evaluation.templateBuilder.criterionLabelPlaceholder', 'Criterion label')}
                    ...
                    <textarea
                      ...
                      placeholder={t('common.evaluation.templateBuilder.criterionDescriptionPlaceholder', 'Description shown to the evaluator (optional)')}
                      rows={2}
              ...
              >
                <Plus className="w-4 h-4" /> {t('common.evaluation.templateBuilder.addCriterion', 'Add Criterion')}
              </button>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => setShowEditor(false)} className="min-h-11 px-4 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50">
                {t('common.evaluation.templateBuilder.back', '← Back')}
              </button>
              <button
                type="button"
              ...
              >
                {saving
                  ? t('common.evaluation.templateBuilder.saving', 'Saving…')
                  : editing
                    ? t('common.evaluation.templateBuilder.saveChanges', 'Save Changes')
                    : t('common.evaluation.templateBuilder.createTemplate', 'Create Template')}
              </button>
            </div>
          </div>
        )}
