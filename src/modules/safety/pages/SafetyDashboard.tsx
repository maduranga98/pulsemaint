...
const field = 'rounded-lg border border-[#1E3A5F] bg-[#0A1628] px-2.5 py-1.5 text-xs text-[#F0F4F8] outline-none focus:border-[#1A56DB]';
...
export default function SafetyDashboard() {
  ...
  // Shared date range for both date-filterable widgets below — empty means
  ...
  // Cases nobody has acted on yet — the ones a safety officer most needs to
  // see, instead of just the most recently reported.
  const unactionedCases = useMemo(
    () => dateFilteredCases.filter((c) => (c.actions ?? []).length === 0),
    [dateFilteredCases],
  );
  ...
  const severityData = useMemo(() => {
    ...
    dateFilteredCases.forEach((c) => counts.set(c.severity, (counts.get(c.severity) ?? 0) + 1));
  ...
  return (
    <div className="min-h-full bg-[#0A1628] text-[#F0F4F8]">
      ...
      <div className="space-y-6 px-4 pb-10 sm:px-6 lg:px-8">
        ...
        {/* Shared date filter for the widgets below */}
        <div className="flex flex-wrap items-center gap-2">
          <label className="text-xs text-[#8BA3BF]">From</label>
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className={field} />
          <label className="text-xs text-[#8BA3BF]">To</label>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <DashboardWidget title="Cases by Severity">
            {severityData.length === 0 ? (
              <EmptyState message="No safety cases in this range" />
            ) : (
              <div className="h-56">
                <ResponsiveContainer>
                  <PieChart>
        ...
        {/* Cases with no action taken yet */}
        <DashboardWidget title={`Cases Reported (${unactionedCases.length})`}>
          {unactionedCases.length === 0 ? (
            <EmptyState message="No unactioned cases in this range" />
          ) : (
                ...
                <tbody>
                  {unactionedCases.map((c) => (
                    <tr key={c.id} className="border-b border-[#1E3A5F]/50">
                      ...
                      <td className="py-2 text-xs text-[#8BA3BF]">{c.reportedAt?.toDate?.().toLocaleDateString?.() ?? ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
