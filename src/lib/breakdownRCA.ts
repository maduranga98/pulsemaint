...
function heuristicSuggestion(input: RCAInput): BreakdownRCASuggestion {
  ...
  const rules = RULES_BY_TYPE[input.breakdownType] ?? [];
...
function buildPrompt(input: RCAInput, history: Breakdown[]): string {
...
Failure type: ${input.breakdownType}
Severity: ${input.severity}
Reported description: ${input.description || '(not provided)'}
...
Recent breakdown history for this same machine (most recent first):
${historyBlock}

Based on this, return the most probable root causes (ranked, most likely first) and concrete recommended next diagnostic/repair actions for the technician. Be specific to the failure type and reuse patterns visible in the history when they recur. Keep each item to one concise sentence. Return 2-4 items per list.`;
}

/**
 * Suggests probable root causes and next actions for a breakdown assessment.
 * Uses Gemini when VITE_GEMINI_API_KEY is configured, falling back to a
 * deterministic keyword engine (mirrors the Audit module's approach) when
...
export async function suggestBreakdownRootCause(
  ...
  try {
    const result = await generateGeminiJson<{ probableCauses: string[]; recommendedActions: string[] }>(
      buildPrompt(input, history),
      {
