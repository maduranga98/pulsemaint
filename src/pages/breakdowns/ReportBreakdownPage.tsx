...
export default function ReportBreakdownPage() {
  ...
  if (submittedTicket) {
    // Explicit dark-palette hex classes here (matching the sheets/drawers
    // elsewhere in the app, e.g. TechnicianWOExecutionSheet) rather than the
    // usual light Tailwind classes (bg-white, text-slate-900, ...) that the
    // app-wide .app-main-dark stylesheet auto-converts — that override only
    // matches plain utility classes, not the bg-indigo-50/50 opacity variant
          ...
          <div className="rounded-xl border border-[#1E3A5F] bg-[#0A1628] p-4 space-y-3">
            ...
            {!suggestionLoading && suggestion && (
              <>
                ...
                <p className="text-[11px] text-[#6B7F9A]">
