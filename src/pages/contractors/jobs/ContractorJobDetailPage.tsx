...
export function ContractorJobDetailPage() {
  ...
  if (loading) return <div className="p-6 text-slate-500">{t('common.contractors.jobs.detailPage.loading')}</div>;
  if (!job) return <div className="p-6 text-slate-500">{t('common.contractors.jobs.detailPage.notFound')}</div>;
  ...
  return (
    <div className="space-y-5 p-4 lg:p-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="font-semibold text-slate-950">{t('common.contractors.jobs.detailPage.team.title')}</h2>
        ...
        <p className="text-sm text-slate-500">{job.technicianNames.join(', ') || t('common.contractors.jobs.detailPage.team.techniciansNotConfirmed')}</p>
      </section>
      ...
      <div className="flex flex-wrap gap-2">
        <Link to={`/app/contractors/jobs/${job.id}/log-work`} className="rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700">{t('common.contractors.jobs.detailPage.actions.logWork')}</Link>
        <Link to={`/app/contractors/jobs/${job.id}/sign-off`} className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">{t('common.contractors.jobs.detailPage.actions.signOff')}</Link>
        <Link to={`/app/contractors/jobs/${job.id}/invoice`} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white">{t('common.contractors.jobs.detailPage.actions.invoice')}</Link>
        <Link to={`/app/contractors/jobs/${job.id}/rate`} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-semibold text-white">{t('common.contractors.jobs.detailPage.actions.rate')}</Link>
      </div>
    </div>
  );
}
