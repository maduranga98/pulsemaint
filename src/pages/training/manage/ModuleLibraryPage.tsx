  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-xl font-bold text-slate-900">{t('common.trainingShared.manager.moduleLibrary.title')}</h1>
        {canAuthor && (
          <button
            onClick={() => navigate('/app/training/manage/modules/new')}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
          >
            <Plus size={16} /> {t('common.trainingShared.manager.moduleLibrary.createModule')}
          </button>
        )}
      </div>
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {(
          [
            { value: 'modules', label: t('common.trainingShared.manager.moduleLibrary.tabs.modules') },
            { value: 'assigned', label: t('common.trainingShared.manager.moduleLibrary.tabs.assigned') },
