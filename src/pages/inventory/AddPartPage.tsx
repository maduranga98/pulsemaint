...
export function AddPartPage() {
  ...
  async function onSubmit(values: CreatePartFormValues) {
    ...
    try {
      ...
      addToast(t('common.inventory.addPartPage.toasts.createSuccess', { partNumber }), 'success');
      ...
      if (warrantyFiles.length > 0) {
        try {
          ...
          addToast(t('common.inventory.addPartPage.toasts.warrantyUploadFailed'), 'warning');
        }
      }
      ...
      addToast(t('common.inventory.addPartPage.toasts.createFailed', { message }), 'error');
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('common.inventory.addPartPage.title')}</h1>
      </div>

      <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-5" noValidate>
        {/* 1. Identification */}
        <SectionCard title={t('common.inventory.addPartPage.sections.identification')}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label={t('common.inventory.partForm.fields.partNumber')}>
              <input
                readOnly
                disabled
                value={
                  selectedCategory
                    ? t('common.inventory.partForm.fields.partNumberAutoGenerated', { format: `${categoryPrefixLetter(selectedCategory)}-xxx-xxx-0001` })
                    : t('common.inventory.partForm.fields.partNumberSelectCategory')
                }
                className={`${inputCls} bg-gray-50 text-gray-500`}
              />
              <p className="text-xs text-gray-400 mt-1">
                {t('common.inventory.partForm.fields.partNumberHint')}
              </p>
            </Field>
            <Field label={t('common.inventory.partForm.fields.name')} required error={errors.name?.message}>
              <input {...register('name')} className={inputCls} placeholder={t('common.inventory.partForm.fields.namePlaceholder')} />
            </Field>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <select {...field} className={inputCls}>
                    <option value="">{t('common.inventory.partForm.fields.unitSelect')}</option>
                    {UNITS.map((u) => (
            ...
            <Field label={t('common.inventory.partForm.fields.status')} error={errors.status?.message}>
              <Controller
                name="status"
                control={control}
                render={({ field }) => (
            ...
            <Field label={t('common.inventory.partForm.fields.criticality')} error={errors.criticality?.message}>
              <Controller
                name="criticality"
                control={control}
                render={({ field }) => (
        ...
        {/* 2. Stock Levels */}
        <SectionCard title={t('common.inventory.addPartPage.sections.stockLevels')}>
          ...
          <Field label={t('common.inventory.partForm.fields.storeLocation')}>
            <input {...register('storeLocation')} className={inputCls} placeholder={t('common.inventory.partForm.fields.storeLocationPlaceholder')} />
          </Field>
          <div className="pt-2">
            <p className="text-xs text-gray-500 mb-2">{t('common.inventory.partForm.fields.stockPreview')}</p>
            <StockGauge
        ...
        {/* 3. Details */}
        <SectionCard title={t('common.inventory.addPartPage.sections.details')}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label={t('common.inventory.partForm.fields.brand')}>
              <input {...register('brand')} className={inputCls} placeholder={t('common.inventory.partForm.fields.brandPlaceholder')} />
            </Field>
            <Field label={t('common.inventory.partForm.fields.modelRef')}>
              <input {...register('modelRef')} className={inputCls} placeholder={t('common.inventory.partForm.fields.modelRefPlaceholder')} />
            </Field>
          ...
          <Field label={t('common.inventory.partForm.fields.description')}>
            <textarea {...register('description')} rows={3} className={`${inputCls} resize-none`} placeholder={t('common.inventory.partForm.fields.descriptionPlaceholder')} />
          </Field>
          ...
          <Field label={t('common.inventory.partForm.fields.warrantyFiles')}>
            ...
            {warrantyFiles.length > 0 && (
              <p className="text-xs text-gray-500 mt-1">{t('common.inventory.partForm.fields.filesSelected', { count: warrantyFiles.length })}</p>
            )}
          </Field>
        </SectionCard>

        {/* 4. Supplier & Pricing */}
        <SectionCard title={t('common.inventory.addPartPage.sections.supplierPricing')}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {canViewCost && (
              <Field label={t('common.inventory.partForm.fields.unitCost')} error={errors.unitCost?.message}>
            ...
            <Field label={t('common.inventory.partForm.fields.supplierPartCode')}>
              <input {...register('supplierPartCode')} className={inputCls} placeholder={t('common.inventory.partForm.fields.supplierPartCodePlaceholder')} />
            </Field>
        ...
        {/* 5. Notes */}
        <SectionCard title={t('common.inventory.addPartPage.sections.notes')}>
          <Field label={t('common.inventory.partForm.fields.notes')}>
            <textarea {...register('notes')} rows={4} className={`${inputCls} resize-none`} placeholder={t('common.inventory.partForm.fields.notesPlaceholder')} />
          </Field>
        </SectionCard>
        ...
        <div className="flex flex-wrap gap-3 pb-8">
          ...
          >
            ...
            {t('common.inventory.addPartPage.actions.savePart')}
          </button>
          <button
            type="submit"
            disabled={saving}
