...
const criticalityConfig: Record<MachineCriticality, { color: string; bg: string }> = {
  1: { color: 'text-blue-600', bg: 'bg-blue-50' },
  2: { color: 'text-green-600', bg: 'bg-green-50' },
  3: { color: 'text-cyan-600', bg: 'bg-cyan-50' },
  4: { color: 'text-amber-600', bg: 'bg-amber-50' },
  5: { color: 'text-red-600', bg: 'bg-red-50' },
};
...
export function MachineCriticalityBadge({
  ...
  return (
    ...
    >
      {t(`common.machines.criticalityLabels.${criticality}`)}
    </div>
  );
}
