  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-semibold text-slate-950">{t('common.contractors.jobs.photos.title')}</h2>
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700">
          ...
          {t('common.contractors.jobs.photos.addPhotos')}
          <input type="file" accept="image/*" multiple className="hidden" />
        </label>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-6">
        {photos.length ? photos.map((photo) => <img key={photo} src={photo} alt="" className="aspect-square rounded-md object-cover" />) : <p className="col-span-full text-sm text-slate-500">{t('common.contractors.jobs.photos.empty')}</p>}
      </div>
    </section>
  );
}
