...
export function ContractorFormLayout({ mode }: ContractorFormLayoutProps) {
  ...
  async function saveContractor(formEl: HTMLFormElement): Promise<string | null> {
    if (!companyId) {
      setError(t('common.contractors.registry.formLayout.errors.missingCompanyContext'));
      return null;
    }
    ...
    if (!payload.companyName || !payload.registrationNumber || !payload.primaryContactName) {
      setError(t('common.contractors.registry.formLayout.errors.missingRequiredFields'));
      return null;
    }
  ...
  async function handleSave(e: FormEvent<HTMLFormElement>, openDocsAfter: boolean) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('common.contractors.registry.formLayout.errors.saveFailed'));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (mode === 'edit' && loadingContractor) {
    return <div className="p-6 text-slate-500">{t('common.contractors.registry.formLayout.loading')}</div>;
  }
  if (mode === 'edit' && !contractor) {
    return <div className="p-6 text-slate-500">{t('common.contractors.registry.formLayout.notFound')}</div>;
  }
    ...
    >
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        <aside className="hidden rounded-lg border border-slate-200 bg-white p-3 lg:block lg:sticky lg:top-4 lg:self-start">
          {(['company', 'contacts', 'specializations', 'financial', 'documents'] as const).map((key, index) => (
            <button
              key={key}
              type="button"
            ...
            >
              {t(`common.contractors.registry.formStepper.steps.${key}`)}
            </button>
          ))}
        </aside>
      ...
      <footer className="sticky bottom-0 flex flex-col gap-2 border-t border-slate-200 bg-slate-50 p-3 sm:flex-row sm:justify-end">
        <Link to="/app/contractors" className="rounded-md border border-slate-200 bg-white px-4 py-2.5 text-center text-sm font-semibold text-slate-700">
          {t('common.contractors.registry.formLayout.actions.cancel')}
        </Link>
        {mode === 'new' && (
          <button
            type="button"
          ...
          >
            {submitting ? t('common.contractors.registry.formLayout.actions.saving') : t('common.contractors.registry.formLayout.actions.saveAndAddDocuments')}
          </button>
        )}
        <button
          type="submit"
        ...
        >
          {submitting ? t('common.contractors.registry.formLayout.actions.saving') : mode === 'new' ? t('common.contractors.registry.formLayout.actions.saveContractor') : t('common.contractors.registry.formLayout.actions.updateContractor')}
        </button>
      </footer>
    </form>
  );
