  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white">
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-semibold text-slate-800">
        {t('common.contractors.documents.historyAccordion.title', { documentType: DOCUMENT_TYPE_LABELS[documentType] })}
          ...
          {documents.map((document) => (
            <div key={document.id} className="px-4 py-3 text-sm text-slate-600">
              <span className="font-medium text-slate-900">
                {t('common.contractors.documents.historyAccordion.version', { version: document.version })}
              </span>{' '}
              - {document.documentName} - {document.uploadedAt.toDate().toLocaleDateString()}
            </div>
          ))}
        </div>
      )}
