              ...
              <div key={c.id} className="bg-[#0A1628] rounded-lg border border-[#1E3A5F] overflow-hidden">
                ...
                {isOpen && (
                  <div className="px-3 pb-3 space-y-2.5 border-t border-[#1E3A5F] pt-2.5">
                    {c.description && (
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-[#8BA3BF]">Description</p>
                        <p className="text-sm text-[#F0F4F8] whitespace-pre-wrap">{c.description}</p>
                      </div>
                    )}
                    {c.location && (
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-[#8BA3BF]">Location</p>
                        <p className="text-sm text-[#F0F4F8]">{c.location}</p>
                      </div>
                    )}
                    {c.actionToTake && (
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-300">What You Need To Do</p>
                    ...
                    <div>
                      <p className="text-[11px] font-semibold uppercase tracking-wide text-[#8BA3BF]">
                        Actions Taken {c.actions.length > 0 ? `(${c.actions.length})` : ''}
                      </p>
                      {c.actions.length === 0 ? (
                        <p className="text-sm text-[#8BA3BF] italic">No actions logged yet.</p>
                      ) : (
                        <ul className="space-y-1.5 mt-1">
                          {c.actions.map((a, i) => (
                            <li key={i} className="text-sm text-[#F0F4F8]">
                              ...
                              {a.newStatus && <span className="text-[#8BA3BF]"> — moved to {a.newStatus}</span>}
