...
export default function ReportBreakdownWidget() {
          ...
          async (decodedText) => {
            ...
            try {
              ...
              if (machineId) {
                ...
                setScanError(t('common.breakdowns.errors.invalidQrMachine'));
              }
            } catch {
              setScanError(t('common.breakdowns.floorOperatorWidgets.notMachineQr'));
            }
          },
          () => {},
        );
      } catch (err: any) {
        setScanError(err?.message || t('common.breakdowns.errors.cameraOpenFailed'));
        setShowScanner(false);
      }
    }, 100);
  }
  ...
  return (
    <DashboardWidget title={t('common.breakdowns.floorOperatorWidgets.reportWidgetTitle')}>
      {scanError && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          ...
          {t('common.breakdowns.floorOperatorWidgets.scanQrCode')}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md mx-4 p-6">
            ...
            <p className="text-xs text-slate-500 mt-3 text-center">
              {t('common.breakdowns.qrScanner.hint')}
            </p>
          </div>
        </div>
      )}
