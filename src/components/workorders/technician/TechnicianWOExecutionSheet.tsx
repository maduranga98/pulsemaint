        ...
        <div className="border-b border-[#1E3A5F] p-4">
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            <span className="text-xs text-[#8BA3BF]">
              {t('common.workOrders.technicianSheet.supervisorLabel', { name: wo.supervisorInChargeName })}
            </span>
          </div>
          ...
          {isOverdue && (
            <div className="mt-3 rounded-lg border border-red-500/40 bg-red-500/10 px-3 py-2">
              <p className="text-xs text-red-300">{t('common.workOrders.technicianSheet.overdueNotice')}</p>
            </div>
          )}
        </div>
            ...
            <div className="space-y-5">
              {canStart && !safetyPreview && (
                <div className="space-y-3">
                    ...
                    <div className={`rounded-lg border p-3 ${workPermitActive ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-orange-500/40 bg-orange-500/10'}`}>
                      {workPermits.length > 0 ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-xs text-orange-300">{t('common.workOrders.technicianSheet.noWorkPermitFound')}</p>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  >
                    <Play className="h-5 w-5" /> {t('common.workOrders.technicianSheet.startWOButton')}
                  </button>
                  <p className="text-center text-[11px] text-[#8BA3BF]">
                    {!workPermitActive
                      ? t('common.workOrders.technicianSheet.workPermitRequiredHint')
                      : t('common.workOrders.technicianSheet.confirmSafetyHint')}
                  </p>
                  ...
                  {assignerDocuments.length > 0 && (
                    <div>
                      <h3 className="mb-2 text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.documentsFromAssignerTitle')}</h3>
                  ...
                  {wo.checklist.length > 0 && (
                    <div>
                      <h3 className="mb-2 text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.yourTasksTitle')}</h3>
                      {wo.specialToolsRequired && (
                        <div className="mb-3 rounded-lg border border-[#1E3A5F] bg-[#0F1E35] px-3 py-2.5">
                          <p className="text-xs font-semibold uppercase tracking-wide text-[#8BA3BF]">{t('common.workOrders.technicianSheet.specialToolsRequiredTitle')}</p>
                      ...
                      <p className="mt-1.5 text-center text-[11px] text-[#8BA3BF]">
                        {t('common.workOrders.technicianSheet.startWOToRecordHint')}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="rounded-lg bg-white p-4">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-gray-900">{t('common.workOrders.technicianSheet.safetyPrecautionsTitle')}</h3>
                    </div>
                    <p className="mb-3 text-sm text-gray-600">
                      {t('common.workOrders.technicianSheet.safetyPrecautionsHint')}
                    </p>
                    {wo.ptwCategory && (
                      <p className="mb-3 text-xs text-gray-500">
                        {t('common.workOrders.technicianSheet.permitCategoryPrefix')}{' '}
                        <span className="font-medium text-gray-700">{wo.ptwCategory}</span>
                      </p>
                    )}
                    <label className="flex cursor-pointer items-start gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-800">
                        {t('common.workOrders.technicianSheet.safetyConfirmLabel')}
                      </span>
                    </label>
                  </div>
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#1A56DB] px-4 py-3 font-semibold text-white hover:bg-[#1648b8] disabled:opacity-50"
                  >
                    <MapPin className="h-5 w-5" /> {t('common.workOrders.technicianSheet.confirmSafetyAndStartButton')}
                  </button>
                  <button
                    onClick={() => {
                  ...
                  >
                    {t('common.workOrders.technicianSheet.backButton')}
                  </button>
                  {!safetyConfirmed && (
                    <p className="text-center text-[11px] text-[#8BA3BF]">
                      {t('common.workOrders.technicianSheet.selectConfirmationHint')}
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div className="rounded-lg border border-orange-500/40 bg-orange-500/10 p-3">
                    <p className="text-sm font-semibold text-orange-300">
                      {myHoldStatus === 'ON_HOLD_PARTS'
                        ? t('common.workOrders.technicianSheet.pausedWaitingForParts')
                        : t('common.workOrders.technicianSheet.pausedWaitingForApproval')}
                    </p>
                  </div>
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-[#1A56DB] px-4 py-3 font-semibold text-white hover:bg-[#1648b8] disabled:opacity-50"
                  >
                    <Play className="h-5 w-5" /> {t('common.workOrders.technicianSheet.resumeButton')}
                  </button>
                </div>
              )}
              ...
              {(isInProgress || isOnHold) && assignerDocuments.length > 0 && (
                <div>
                  <h3 className="mb-2 text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.documentsTitle')}</h3>
                  <div className="space-y-2">
                    {assignerDocuments.map((d) => (
                      <a
                        key={d.id}
                ...
                <>
                  {isInProgress && (
                    <div className="rounded-lg border border-[#1E3A5F] bg-[#0F1E35] p-3 text-center">
                      <p className="text-xs uppercase tracking-wide text-[#8BA3BF]">{t('common.workOrders.technicianSheet.timeOnJobLabel')}</p>
                      <p className="font-mono text-2xl font-bold text-[#00C2FF]">{formatElapsed(elapsed)}</p>
                    </div>
                  )}

                  <div>
                    <h3 className="mb-2 text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.checklistTitle')}</h3>
                    {wo.specialToolsRequired && (
                      <div className="mb-3 rounded-lg border border-[#1E3A5F] bg-[#0F1E35] px-3 py-2.5">
                        <p className="text-xs font-semibold uppercase tracking-wide text-[#8BA3BF]">{t('common.workOrders.technicianSheet.specialToolsRequiredTitle')}</p>
                        <p className="mt-0.5 text-sm text-[#F0F4F8]">{wo.specialToolsRequired}</p>
                  ...
                  {isInProgress && (
                    <>
                      <div>
                        <h3 className="mb-2 text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.fieldMediaTitle')}</h3>
                      ...
                      >
                        <PackagePlus className="h-4 w-4 text-[#00C2FF]" /> {t('common.workOrders.technicianSheet.requestInventoryButton')}
                      </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        >
                          <PackageX className="h-4 w-4 text-orange-400" /> {t('common.workOrders.technicianSheet.holdPartsButton')}
                        </button>
                        <button
                        ...
                        >
                          <Pause className="h-4 w-4 text-red-400" /> {t('common.workOrders.technicianSheet.requestApprovalButton')}
                        </button>
                      </div>

                      {showApprovalRequest && (
                        <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-3 space-y-2">
                          <p className="text-xs font-semibold uppercase tracking-wide text-red-300">{t('common.workOrders.technicianSheet.requestApprovalTitle')}</p>
                          <p className="text-xs text-[#8BA3BF]">
                            {t('common.workOrders.technicianSheet.requestApprovalHint')}
                          </p>
                          <textarea
                            value={approvalNote}
                      ...
                      {myPendingApprovalRequest && (
                        <div className="rounded-lg border border-orange-500/40 bg-orange-500/10 p-3 space-y-1">
                          <p className="text-xs font-semibold uppercase tracking-wide text-orange-300">{t('common.workOrders.technicianSheet.approvalRequestedPendingTitle')}</p>
                          <p className="text-sm text-[#F0F4F8]">{myPendingApprovalRequest.note}</p>
                          <p className="text-xs text-[#8BA3BF]">
                            {t('common.workOrders.technicianSheet.requestedAtLabel', {
                              date: myPendingApprovalRequest.requestedAt?.toDate ? myPendingApprovalRequest.requestedAt.toDate().toLocaleString() : '',
                            })}
                          </p>
                        </div>
                      )}
                          ...
                          {myResolvedApprovalRequests.map((r) => (
                            ...
                            >
                              <p className={`text-xs font-semibold uppercase tracking-wide ${r.status === 'approved' ? 'text-emerald-300' : 'text-red-300'}`}>
                                {r.status === 'approved'
                                  ? t('common.workOrders.technicianSheet.requestApprovedBy', { name: r.resolvedByName })
                                  : t('common.workOrders.technicianSheet.requestRejectedBy', { name: r.resolvedByName })}
                              </p>
                      ...
                      {assigneeRows.length > 1 && (
                        <div className="rounded-lg border border-[#1E3A5F] bg-[#0F1E35] p-3">
                          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8BA3BF]">
                            {t('common.workOrders.technicianSheet.teamProgressLabel', {
                              completed: completedCount,
                              total: assigneeRows.length,
                            })}
                          </p>
                            ...
                            {assigneeRows.map((a) => (
                              <div key={a.id} className="flex items-center justify-between text-sm">
                                <span className={a.id === myId ? 'font-semibold text-[#F0F4F8]' : 'text-[#F0F4F8]'}>
                                  {a.name}{a.id === myId ? t('common.workOrders.technicianSheet.youSuffix') : ''}
                                </span>
                                <span className={`text-xs ${a.done ? 'text-emerald-400' : 'text-[#8BA3BF]'}`}>
                                  {a.done ? t('common.workOrders.technicianSheet.completedStatus') : t('common.workOrders.technicianSheet.inProgressStatus')}
                                </span>
                              </div>
                            ))}
                            <div className="flex items-center justify-between border-t border-[#1E3A5F] pt-1.5 text-sm">
                              <span className="text-[#8BA3BF]">{t('common.workOrders.technicianSheet.supervisorSignOffLabel')}</span>
                              <span className={`text-xs ${wo.supervisorSignOffAt ? 'text-emerald-400' : 'text-[#8BA3BF]'}`}>
                                {wo.supervisorSignOffAt ? t('common.workOrders.technicianSheet.signedOffStatus') : t('common.workOrders.technicianSheet.pendingStatus')}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        myCompletion ? (
                          <div className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 p-3">
                            <p className="flex items-center gap-2 text-sm font-semibold text-emerald-300">
                              <CheckCircle2 className="h-4 w-4" /> {t('common.workOrders.technicianSheet.youCompletedWork')}
                            </p>
                          ...
                          <div className="rounded-lg border border-[#1E3A5F] bg-[#0F1E35] p-3 space-y-2">
                            <p className="text-sm font-semibold text-[#F0F4F8]">{t('common.workOrders.technicianSheet.completeMyWorkTitle')}</p>
                            {myCompletedSteps && (
                            ...
                            <textarea
                              ...
                              placeholder={t('common.workOrders.technicianSheet.workDonePlaceholder')}
                            ...
                            <div className="flex gap-2">
                              ...
                              >
                                <CheckCircle2 className="h-4 w-4" /> {t('common.workOrders.technicianSheet.submitMyWorkButton')}
                              </button>
                              <button
                          ...
                          >
                            <CheckCircle2 className="h-5 w-5" /> {t('common.workOrders.technicianSheet.completeMyWorkButton')}
                          </button>
                        )
                      )}
                      ...
                      {everyoneDone ? (
                        ...
                        >
                          <CheckCircle2 className="h-5 w-5" /> {t('common.workOrders.technicianSheet.submitAndCompleteButton')}
                        </button>
                      ) : (
                        <p className="text-center text-[11px] text-[#8BA3BF]">
                          {t('common.workOrders.technicianSheet.pendingTeamCompletionHint', {
                            completed: completedCount,
                            total: assigneeRows.length,
                          })}
                        </p>
              ...
              {!canStart && !isInProgress && !isOnHold && (
                <div className="flex flex-col items-center gap-2 py-10 text-center text-[#8BA3BF]">
                  ...
                  <p className="text-sm">
                    {t('common.workOrders.technicianSheet.statusHint', { status: wo.status.replace(/_/g, ' ').toLowerCase() })}
                  </p>
                </div>
              )}
            </div>
