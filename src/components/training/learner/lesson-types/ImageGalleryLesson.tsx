  ...
  if (images.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 h-64 px-4 text-center">
        <p className="text-slate-400 text-sm">{t('common.trainingShared.imageGalleryLesson.noImages')}</p>
        {progress?.completed ? (
          <div className="flex items-center gap-2 text-green-600 font-medium">
            <CheckCircle size={18} />
            {t('common.trainingShared.imageGalleryLesson.completed')}
          </div>
        ) : (
          <button
          ...
          >
            <CheckCircle size={18} />
            {t('common.trainingShared.imageGalleryLesson.markComplete')}
          </button>
        )}
      </div>
    );
      ...
      >
        ...
        {currentIndex > 0 && (
          <button
            ...
            aria-label={t('common.trainingShared.imageGalleryLesson.previousImage')}
          >
            <ChevronLeft size={20} />
          </button>
        )}
        {currentIndex < images.length - 1 && (
          <button
            ...
            aria-label={t('common.trainingShared.imageGalleryLesson.nextImage')}
          >
        ...
        <div className="absolute top-3 right-3 bg-black/60 text-white text-xs px-2 py-1 rounded-full">
          {t('common.trainingShared.imageGalleryLesson.counter', { current: currentIndex + 1, total: images.length })}
        </div>
      </div>
      ...
      <div className="bg-white px-4 py-3 space-y-3">
        ...
        <div className="flex justify-center gap-1.5">
          {images.map((_, i) => (
            <button
              ...
              aria-label={t('common.trainingShared.imageGalleryLesson.goToImage', { index: i + 1 })}
            />
          ))}
        </div>
        ...
        {!viewedAll && (
          <p className="text-xs text-slate-400 text-center">
            {t('common.trainingShared.imageGalleryLesson.viewAllHint')}
          </p>
        )}
      </div>
    </div>
