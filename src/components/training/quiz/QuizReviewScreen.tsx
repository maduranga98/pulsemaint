...
export default function QuizReviewScreen({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900">{t('common.trainingShared.quiz.review.title')}</h2>
        <p className="text-slate-500 text-sm mt-1">
          {t('common.trainingShared.quiz.review.answeredCount', { answered: answered.length, total: questions.length })}
        </p>
      </div>
      ...
      {unanswered.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 text-amber-800 text-sm">
          ...
          <span>{t('common.trainingShared.quiz.review.unansweredWarning', { count: unanswered.length })}</span>
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <span className="text-sm text-slate-700 line-clamp-1">
                {t('common.trainingShared.quiz.answerReview.questionLabel', { number: i + 1 })}: {q.text}
              </span>
              <span className={`ml-auto text-xs font-medium shrink-0 ${isAnswered ? 'text-green-600' : 'text-amber-600'}`}>
                {isAnswered ? t('common.trainingShared.quiz.review.answered') : t('common.trainingShared.quiz.review.skipped')}
              </span>
            </div>
          );
        })}
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          <ChevronLeft size={18} /> {t('common.trainingShared.quiz.review.back')}
        </button>
        <button
        ...
        >
          {t('common.trainingShared.quiz.review.submitQuiz')}
        </button>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
            <h3 className="font-bold text-slate-900 text-lg">{t('common.trainingShared.quiz.review.confirmTitle')}</h3>
            <p className="text-slate-600 text-sm">
              {t('common.trainingShared.quiz.review.confirmBody')}
            </p>
            <div className="flex gap-3">
              ...
              >
                {t('common.trainingShared.quiz.review.cancel')}
              </button>
              <button
              ...
              >
                {isSubmitting ? t('common.trainingShared.quiz.review.submitting') : t('common.trainingShared.quiz.review.submit')}
              </button>
            </div>
          </div>
        </div>
