...
### Staff Requests
Every role except admin raises requests from **My Requests** (`/app/requests`): personal, work, service letter, access to past WO / breakdown details, or other, with attachments. A request goes to a role group: the supervisors of the requester's plant **and** department, the plant manager(s) of the requester's plant, or admin. Supervisors can send to plant manager/admin; plant managers to admin. Handlers reply with text and attachments from the **Requests Inbox** (`/app/requests/inbox`) and can close/reopen; the requester can follow up. Closed requests drop off every list 7 days after closing for every role except plant managers, who keep the full history (hidden only — the documents stay in Firestore). Stored in `staff_requests`; attachments under `companies/{companyId}/staff_requests/{requestId}/` in Storage.

**Record access (past WO / breakdown details).** These requests can only be addressed to a plant manager or admin. The requester picks the record from a searchable list of their own plant's work orders and breakdowns (their own department's too, for technician / trainee / supervisor / floor operator, plus any they were assigned to), shown by number and machine name — served by the `listRecordReferences` callable (only number / machine / date / status, 500 most recent of each), since most roles can't list work orders under the rules. The grantor's share window opens with that record preselected. In the inbox they get **Select records & share** — pick any number of the plant's work orders and/or breakdowns, set a *viewable until* date/time (24 h / 3 d / 7 d / 30 d presets or custom) and an optional note — or **Reject** (optional reason; closes the request). Shared records are copied into `record_access_grants` (one doc per record, with a `snapshot` of the record), so the requester needs no access to `workOrders` / `breakdown_tickets`. The requester sees them read-only (with PDF download) under **Records shared with you** on My Requests and inside the request, until the expiry — then they vanish from the UI immediately and the `purgeExpiredRecordGrants` scheduled function (every 15 min) deletes them. The grantor can revoke a share early from the request.
